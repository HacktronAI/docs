...
<Tabs>
  <Tab title="Claude Code (CLI)">
    ...
    <Accordion title="Prefer using an API key?">
      ...
      ```bash
      claude mcp add --scope user --transport http hacktron https://mcp.hacktron.ai/mcp --header "Authorization: Bearer hacktron_your_key_here"
      ```
...
| --- | --- | --- |
| `list_scans` | `read` | List scans with status/type filters and per-severity counts |
| `get_scan` | `read` | Get a single scan by ID |
| `export_findings` | `read` | Export a scan's findings as JSON, CSV, or SARIF |
| `list_findings` | `read` | List findings with severity/state/scan/repo filters |
| `get_finding` | `read` | Get full finding detail, including the triage thread |
| `update_finding` | `write` | Change a finding's state and/or severity |
| `add_finding_comment` | `write` | Add a comment to a finding's triage thread |

<Note>
  Tools are filtered by your credential's scopes. A read-only credential never sees the `write` tools at all.
</Note>
