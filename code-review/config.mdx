...
Each category (`skip.*`, `include.*`, `fail_on.severity`) is resolved independently, not the whole file at once:
...
| `skip.labels` | the PR/MR carries one of these labels | case-insensitive exact match|
| `skip.keywords` | the PR/MR **title** contains one of these strings | case-insensitive substring |
| `skip.paths` | **every** changed file matches one of these patterns | case-insensitive glob |
| `skip.authors` | the PR/MR was opened by one of these usernames | case-insensitive exact match|
...
<Note>
  ...
  scanned as usual. Patterns use the same glob syntax as `skip.branches` — for
  example `vendor/**`, `**/*.md`, or `docs/**`.
</Note>

<Accordion title="Glob pattern syntax">
  `skip.branches`, `skip.paths`, and `include.branches` (and their org-wide
...
<Accordion title="Matching nested branches with * vs **">
  ...
  Combine `skip` and `include` to exclude a subtree: `include.branches:
  ["release/**"]` with `skip.branches: ["release/legacy/**"]` scans all
...
<Note>
  `skip` and `include` can target the same category. See [glob matching](/code-review/config#matching-nested-branches-with-vs) for pattern syntax.
</Note>
...
<AccordionGroup>
  <Accordion title="My org requires a label to scan PRs, can one repo skip that rule?">
  ...
  <Accordion title="If a repo only sets one filter, does it lose the rest of the org's filters?">
    No — each filter category (`skip.branches`, `skip.labels`, `include.branches`, and so on) is inherited independently.

    Org default: `skip.labels: [wip]`. A repo adds `skip.branches:
    [release/legacy/**]` but doesn't set `skip.labels`. That repo now skips
    legacy release branches **and** still skips `wip`-labelled PRs — the org's
