...
This is separate from [`.hacktron/rules.md`](/code-review/rules), which shapes the _quality_ of a review. `config.yaml` controls _whether_ a PR is scanned and _whether_ its check passes.
...
<Note>
  ...
  contributors are not scanned automatically. On GitLab, a bot or service
  account follows the normal filter and seat rules when it has Developer or
  higher project access. On Bitbucket, a bot or service account follows the
  normal filter and seat rules when its source branch belongs to the target
  repository; fork PRs are not scanned automatically. An eligible bot author can
  take a Code Review seat when auto-assignment is on. Add its username to
  `exclude.authors` to skip its future PRs or MRs.
</Note>
...
<Accordion title="Glob pattern syntax">
  ...
  with literals. Matching is case-insensitive.

| Syntax  | Meaning                                          | Example                      |
| ------- | ------------------------------------------------ | ---------------------------- |
| `*`     | Any number of characters within one path segment | `*-bot`                      |
| `**`    | Any number of characters, including across `/`   | `release/**`                 |
| `?`     | Exactly one character                            | `release/?.x`                |
| `{a,b}` | One of the comma-separated alternatives          | `{dependabot,renovate}[bot]` |

For author names, `*` and `**` usually behave the same because author names do not contain `/`. The characters `[`, `]`, and a leading `!` are treated literally. Bracket classes, negation, nested braces, and extended glob forms such as `@(a|b)` are not supported.

The same safety limits apply to repository and organization settings:

- Up to 50 entries in each filter category
- Up to 100 characters in each pattern
- Up to five `*` characters in each pattern
- Up to 10 comma-separated alternatives in one brace expression

If a repository pattern exceeds these limits, Hacktron treats the repository configuration as invalid and scans with the fail-open behavior described below.
...
| `release/**`        | `release/foo`, `release/foo/bar`              | None              |
...
<Note>
  `exclude` and `include` can target the same category. See [glob pattern
  syntax](/code-review/config#glob-pattern-syntax) for the supported patterns.
</Note>
...
The threshold is **inclusive**: `high` fails the check on `high` _and_ `critical` findings, while `critical` fails only on `critical`.
