...
<Accordion title="Glob pattern syntax">
  `exclude.branches`, `exclude.paths`, `exclude.authors`/`include.authors`,
  and `include.branches` (and their org-wide equivalents) accept glob
  patterns mixed with literals: `*`, `**`, `?`, and `{(a, b)}` brace
  expansion. Matching is case-insensitive. `[`, `]`, and a leading `!` are
  literal, not special syntax. Each category allows up to 50 entries.
</Accordion>
