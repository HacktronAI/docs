...
<Note>
  ...
  higher project access. On Bitbucket, the same applies to PRs from the target
  repository, while fork PRs are not scanned automatically. An eligible bot
  author can take a Code Review seat when auto-assignment is on. Add its
  username to `exclude.authors` to skip its future PRs or MRs.
</Note>
