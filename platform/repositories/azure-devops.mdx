...
Hacktron now has organization-level access to the selected repositories.

## Link member accounts for PR attribution

The organization-level connection lets Hacktron access repositories and receive pull request events. Members who author Azure DevOps pull requests should also link their Microsoft account so Hacktron can attribute those pull requests to the correct Code Review seat.

Go to **Personal settings**, find **Microsoft** under **Connected accounts**, and select **Connect**. Sign in with the Microsoft account whose email matches your Azure DevOps user principal name (UPN).

<Card
  title="Connect your Microsoft account"
  icon="microsoft"
  href="/platform/account-settings#connect-microsoft-for-azure-devops"
>
  Follow the account-linking steps and check the email requirement.
</Card>

Linking an account does not rerun completed reviews. Future activity on an existing Azure DevOps pull request can reconcile its author with the linked Hacktron member.
