...
Findings don't fix themselves, until now. An automation watches for the security findings you care about and acts on them the moment they appear: it starts a fix and tells your team it's handled. Instead of vulnerabilities waiting in a dashboard for someone to get to them, you set the rules once and Hacktron takes it from there.

Automations can be found in the dashboard sidebar. Organization **Admins** create and manage automations; other members can view them.
...
When a new finding is reported, an enabled automation fires if the finding matches its trigger, severity rules, scope rules, and one of its environments (including any branch limits). Firing starts a remediation run, which uses [automation credits](/platform/automations/credits), and sends your notifications.
...
<Info>
  ...
  [`.hacktron/config.yaml`](/code-review/config#auto-remediate-findings) follows
  that block exclusively, including `auto: false`, which turns auto-remediation
  off for that repository. Organization automations apply only to repositories
  without a `remediation` block.
</Info>
