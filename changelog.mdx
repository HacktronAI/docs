...
<Update label="June 23, 2026" tags={["Code Review","Whitebox","Integrations","MCP"]}>
  ...
  **Threat models**: Repository and application threat models now render inline in a reading layout with a source-document file tree and an "On this page" outline, and you can edit them directly with your changes preserved across regenerations. Repositories show a "Pending" badge while a sync runs, the action button reads "Generate" or "Sync" to match the model's state, and when a refreshed model grounds a scan the findings review opens with a short note on why the model changed.
