...
<Note>
  **Cost Estimations** use offset‑based pagination instead, with `limit`
  (default `50`, max `100`) and `offset` (default `0`). This is called out
  explicitly on the [List Cost
  Estimations](/api-reference/cost-estimations/list-cost-estimations) page.
</Note>
