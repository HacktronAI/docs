...
page=1
while : ; do
  curl -s "https://api.hacktron.ai/api/v1/rest/findings?page=$page&limit=100" \
...
```bash
curl "https://api.hacktron.ai/api/v1/rest/findings?severity=high&state=open&sort_by=updated_at&sort_order=DESC&page=1&limit=25" \
  -H "X-Api-Key: $HACKTRON_API_KEY"
```
